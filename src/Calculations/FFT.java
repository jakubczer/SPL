...
public class FFT {
    ...
    /**
     * Checks is a number is a power of two
     *
     * @param  x number to check
     * @return true if x is a power of two, false otherwise
     */
    private static boolean CheckIfPowerOf2(int x)
    {
        return (x > 1) && (x & (x - 1)) == 0;
    }
    /**
    ...
    public static Complex[] cconvolve(Complex[] x, Complex[] y) {
        ...
        // and are powers of 2
        if (x.length != y.length) throw new IllegalArgumentException("Dimentions don't agree");
        if (!(CheckIfPowerOf2(x.length) && CheckIfPowerOf2(y.length)))

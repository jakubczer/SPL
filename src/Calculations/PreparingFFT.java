...
public class PreparingFFT{
    ...
    /**
     ...
     @param x The real input signal without extra zeros.
     @param windowFunction The window's type (RECTANGLE, BARTLETT, HANNING, HAMMING or BLACKMAN).

     @return The completly prepared signal for FFT (from the FFT class).
     */
    public static Complex[] DataPreparingForFFT(double[] x, WindowFunction windowFunction){
        ...
        if(windowFunction == WindowFunction.Bartlett) data_for_calculation = BartlettWindow(x);
        if(windowFunction == WindowFunction.Hanning) data_for_calculation = HanningWindow(x);
        if(windowFunction == WindowFunction.Hamming) data_for_calculation = HammingWindow(x);
        if(windowFunction == windowFunction.Blackman) data_for_calculation = BlackmanWindow(x);
